...
class AWSIoTThing():
    ...
    def __init__(self, THING_NAME, THING_CODE):
        self.region = THING_REGION
...
def aws_iot_core_create_bulk_things():
    ...
    # get all thing types dismiss thingtype (us, hk, ...)
    ...
    response = iot_client.start_thing_registration_task(templateBody=f.read(), 
                                        ...
                                        inputFileKey=OBJ_PROVISION_FILE,
                                        roleArn=ROLE_ARN)
...
def aws_iot_core_create_certificates():
    ...
    for thing in things['thingNames']:
        ...
        f = open(ROOT_DIR + "/secure/keys/private/"+thing+".pem.key", "w+")
        f.write(key_private)
        f.close()

        # Storing the public key
        f = open(ROOT_DIR + "/secure/keys/public/"+thing+".pem.key", "w+")
        f.write(key_public)
        f.close()

        # Storing the certificate
        f = open(ROOT_DIR + "/secure/certificates/"+thing+".pem.crt", "w+")
        f.write(certificate)
        f.close()
